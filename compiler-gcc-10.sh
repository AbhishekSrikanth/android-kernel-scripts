...
# Paths
PATH_TO_KERNEL_SOURCE="${HOME}/eva_kernel_cedric"
PATH_TO_TOOLCHAIN="${HOME}/arm64-gcc"
PATH_TO_ANYKERNEL="${HOME}/AnyKernel3"
DEVICE_NAME="eva"

PATH_TO_COMPILER="${PATH_TO_TOOLCHAIN}/bin/aarch64-elf-"
...
make O=out ARCH=arm64 CROSS_COMPILE=$PATH_TO_COMPILER -j4
...
git --no-pager log --oneline origin/eva-rel/eas+1.3/3.18^..HEAD > "${PATH_TO_ANYKERNEL}/CHANGELOG.txt"
...
zip -r9 "${ZIPNAME}-gcc-10.zip" * -x .git README.md *placeholder *.zip
